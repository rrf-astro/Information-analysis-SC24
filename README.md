...
This repository contains the analysis notebook, the figures of the manuscript,
and the full set of result tables needed to reproduce every reported number.
...
The notebook queries the Mandal (2020) catalogue directly from CDS VizieR
(`J/A+A/640/A78`) via `astroquery`, so an internet connection is required. It
...
- `generate_robustness_report.py` — assembles `robustness_analysis.md` from the
  robustness CSVs. Every value is read from its source file; no number is
  hand-typed, so the report cannot diverge from the tables it summarises.
...
`robustness_analysis.md` is a generated digest of the eight robustness tables
...
- `integrity_report.md` — integrity checks generated during execution.
...
## A note on the mutual-information modulation

The peak-to-trough modulation of I(TSA;PSI) is estimated under six estimators
(see `mi_estimators.csv`). The plug-in estimators carry a positive
discretisation bias that inflates the ratio at fine binning; the plug-in n=10
value of 5.30× reflects that bias, not a larger physical effect, and is not
reported as a physical magnitude. The bias-corrected Kraskov k-NN estimators
give the reported modulation, approximately 1.34× and above.

## Reproducibility note

`requirements.txt` pins the exact versions used to produce the frozen results.
`scipy` is pinned deliberately: its Mann–Whitney U routine has changed across
releases, which can shift the diagnostic `table_min_max.csv` (MW_U by a few
units) without altering any reported scientific conclusion.
...
Cycle 24*, is under submission. A citation entry with DOI will be added here on
acceptance. In the meantime, please cite this repository and its author
(Rafael R. Ferreira) when using the code or derived tables.
